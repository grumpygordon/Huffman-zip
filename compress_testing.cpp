...
void large_verdict(bool answ, double ftime) {
    ...
    if (answ) {
        ...
        std::cout << "[    OK] in " << std::fixed << ftime / CLOCKS_PER_SEC << " sec. ";
        std::cout << cur << " Kb / " << prev << " Kb\n";
    } else {
        std::cout << "[FAILED] Test #" << test_num << "\n";
        std::cout << "Passed " << test_num - pre_ac - 1 << " large tests in \n" << std::fixed
...
void test() {
    ...
    if (pre_ac != test_num) {
        std::cout << "Program failed pre-tests.\nPassed " << pre_ac << " tests.\n";
        return;
    } else {
        std::cout << "Pre-tests are passed.\nRun large tests:\n";
    }
    ...
    std::cout << "Compressor worked (summary): " << all_time / CLOCKS_PER_SEC << " seconds\n";
    std::cout << "Tests checked in " << std::fixed
